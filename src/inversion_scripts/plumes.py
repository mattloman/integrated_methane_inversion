    ...
    * point geometry on WGS84 crs
    ...
    def _grid_datasource(self, datasource):
        ...
        if inout is None:
            ...
            msg = "Unable to detect point sources, " "continuing without plume data."
            warnings.warn(msg)
            return None
    ...
    def get_coords(self):
        ...
        if self.points is None:
            ...
            coords = np.column_stack(
                (self.points.y.values, self.points.x.values)
            ).tolist()
            return coords
...
class PlumeObserver:
    ...
    def __init__(self, myname, config, usecached, cache=None):
        ...
        if self.usecached:
            msg = f"Using cached plume data for {self.myname}"
            print(msg)
    ...
    def _cache_data(self):
        ...
        self.gdf.to_file(f"{write_dir}/plumes.geojson", driver="GeoJSON")
...
class SRON(PlumeObserver):
    ...
    def _get_data(self):
        ...
        df["emission_rate_std"] = np.where(
            np.isnan(df["uncertainty_t/h"]), df["uncertainty"], df["uncertainty_t/h"]
        )
        ...
        gdf = gdf[
            ["emission_rate", "emission_rate_std", "time", "instrument", "geometry"]
        ]
...
class IMEO(PlumeObserver):
    ...
    def _get_data(self):
        ...
        try:
            ...
            gdf = gdf.rename({"geometry": "plume_geometry"}, axis=1).set_geometry(
                points
            )
